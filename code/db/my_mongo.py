...
import sys
import os  # these you almost always have...
...
class MyMongo(object):
    ...
    def get_one_article(self):
        ...
        '''
        # db.articles.findOne({'raw_html':{$exists:true}},{'url':1,'raw_html':1,
        # _id:0})
    ...
    def get_article_attri(self, testing=0):
        ...
        query = {self.link_url_field_name:
                 {'$exists': 1}, 'title': {'$exists': 1}}
        projection = {
            "_id": 0, 'title': 1, self.link_url_field_name: 1, 'dt': 1}
        if testing:
    ...
    def get_article_body_text(self, testing=0):
        ...
        if testing:
            articles = self.articles_collection.find(
                query, projection=projection, limit=25)
        else:
            articles = self.articles_collection.find(
