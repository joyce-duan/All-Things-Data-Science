...
#def get_articles_nytimes( links_collection, articles_collection, query = query):
...
def get_articles_nytimes(links_collection, articles_collection,link_url_field = link_url_field_default, query = link_query_default):
	...
	counter_inserted = 0
	# need to dedupe???
	#links_no_content = list(links_collection.find({'gothtml': {$exists: false}} ))
	...
	#for i, article in enumerate(links_collection.find()):
		...
		if response:
			...
			try:
				...
				articles_collection.update({'url':url}, {'$set': {'body_text': body}},multi = True)
				counter_inserted += 1
			except DuplicateKeyError:
...
if __name__ == '__main__':
	...
	links_collection, articles_collection = get_db_collections(links_collection_name, articles_collection_name)
	...
	get_articles_nytimes(links_collection, articles_collection, link_url_field = link_url_field_default, query = link_query)
