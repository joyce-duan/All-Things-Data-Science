...
def to_exclude(url):
    ...
    # np.logical_or(in_exclusion_list)
    return [t for t in in_exclusion_list if t]
...
def get_links_from_dir():
    ...
    for fname in fnames:  # [0:1]:  for testing
        ...
        y = parts[0]
        m = parts[1]
        dt_submit = datetime.datetime(
            int(y), int(m), int(d)).strftime('%Y-%m-%d')
        # print dt_submit
...
def get_links_from_dir_linkedin():
    ...
    for fname in fnames:  # [0:1]:  for testing
        ...
        insert_links(
            links_collection, links, linksource='linkedin', dt_submit=dt_submit)
