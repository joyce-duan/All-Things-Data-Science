...
def get_links_to_weekly():
	urls_weekly = []
	dates = []
	fname = '/Users/joyceduan/data/datascienceweekly/' + 'ds_weekly_archive.html'
	...
	for url in urls_all:
		...
		if 'issue' in txt.lower() and txt[:len('Data Science Weekly Newsletter')] == 'Data Science Weekly Newsletter':
			...
			dt_submitted = ''
			try:
				dt_submitted = datetime.datetime(int(str_dt[2][:4]),m, d).strftime('%Y-%m-%d')
			except:
				pass
...
if __name__ == '__main__':
	...
	for i, url in enumerate(urls_weekly[1:]):
		...
		if response:
			...
			insert_links(links_collection, links, linksource=linksource, dt_submit=dates[i])
		#except:
	...
	'''
	link_query = {'gothtml':{'$exists': 0}, 'linksource':linksource, 'triedhtml':{'$exists':0}}
