...
class TopicModel(object):
    ...
    def fit_features(self, df, testing=0):
        ...
        self.vectorizer, vectorized_X, tokenized_articles = fit_tfidf(
            txt_list, self.kw_tfidf, self.func_tokenizer, self.func_stemmer)
...
def plot_mds(data_m, title, distance_metric):
    ...
    plt.scatter(X_dist_transformed[:, 0], X_dist_transformed[
                :, 1], alpha=0.8, edgecolor='none', c=code_for_colors)
    plt.title(title)
    plt.show()
...
def run_model(model_name, kw_tfidf, kw_nmf, func_stemmer, func_tokenizer):
    ...
    topic_model = TopicModel(
        model_name, func_stemmer, func_tokenizer, kw_tfidf, kw_nmf)
    df = read_articles()
...
def tune_hyper_params():
    ...
    func_stemmer = PorterStemmer()
    func_tokenizer = TfidfVectorizer(stop_words='english').build_tokenizer()
    kw_tfidf = {'stop_words': 'english', 'ngram_range': (
        1, 5), 'max_features': 10000, 'min_df': 20, 'max_df': 0.9}
...
def test_run_modeler():
    ...
    df = read_articles()
    W, tokenized_articles, vectorized_X = topic_model.transform_bodytext2topics(
        df.body_text[0:5], 1)
