...
class KmeansTopics(BaseTopics):
    ...
    def set_H(self, centroids, features):
        ...
        for i_centroid, centroid in enumerate(centroids):
            # print 'H shape: %s weights shape: %s i_topic: %i  i_feature: %i
            # j_word: %i' % (self.H.shape, weights.shape, i_topic, i_feature,
            # j_word)
...
def test_run():
    ...
    kmeans_topic_model.set_models(
        centroids, clusters, features, df, num_topics=20, num_words=15)
...
def run_k_means():
    ...
    for i in range(num_clusters):
        ...
        for ind in order_centroids[i, :n_terms]:
            ...
            print (' %s' % features[ind].encode(
                'utf-8', 'ignore')), km.cluster_centers_[i, ind]
