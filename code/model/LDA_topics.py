...
class BaseTopics(object):
    ...
    def get_best_topic_per_article(self):
        ...
        for i_article, topic_w in enumerate(self.W):
            ...
            best_topics_for_articles.append(
                [i_best_topic, topic_w[i_best_topic]])
    ...
    def assign_cluster(self, min_w=0):
        ...
        for i_article in xrange(n_articles):
            if self.best_topics_per_article[i_article, 1] > min_w:
                clusters[i_article] = self.best_topics_per_article[
                    i_article, 0]
    ...
    def plot_hist_weight_best_topic_per_article(self):
        ...
        # plt.show()
        fig.savefig(
            self.model_name + '_hist_best_topic_per_article.png', bbox_inches='tight')
        plt.close(fig)
    ...
    def set_top_topics_terms(self):
        ...
        for i in xrange(n_topics):
            ...
            v = self.H[i].flatten()
            # print type(self.H), type(v), v.shape,
            # type(self.top_word_features)
            ...
            terms_this_topic = zip(
                self.top_word_features[idx_sorted_topk], v[idx_sorted_topk] * 100)
    ...
    def print_topic_results_html(self):
        ...
        with open(model_name + '.html', 'w') as out_fh:
            ...
            for topic_idx, article_w in enumerate(W_t):
                ...
                out_fh.write(
                    '<p><strong>top terms: ' + ' '.join(txt_list) + "</strong></p>\n")
                ...
                for i, idx in enumerate(idx_article_topn):
                    ...
                    out_fh.write('<p> ' + str(i) + '. (%.2f))' %
                                 article_w[idx] + '</p>\n')
                    out_fh.write(
                        '<a href="' + url + '" target="_blank"> %s </a>  <br>\n' % title_this_cleaned)
...
class LDATopics(BaseTopics):
    ...
    def set_models(self, kw_dict, num_topics=20, num_words=15):
        ...
        for k, v in kw_dict.items():
            ...
            if k == 'lda_model':
                self.lda_model = v
                self.lda_topic_matrix = np.array(self.lda_model.show_topics(
    ...
    def set_H(self, lda_topic_matrix):
        ...
        # []  #words used in at least one topic
        self.top_word_features = np.array(list(set(topic_words.flatten())))
        self.top_word_feature_map = dict(
            zip(self.top_word_features, xrange(len(self.top_word_features))))
        ...
        for i_topic, words in enumerate(topic_words):
            for j_word in xrange(len(words)):
                ...
                # print 'H shape: %s weights shape: %s i_topic: %i  i_feature:
                # %i  j_word: %i' % (self.H.shape, weights.shape, i_topic,
                # i_feature, j_word)
    ...
    def set_W(self):
        ...
        for i_article in xrange(self.df.shape[0]):
            w_this = self.lda_model[
                self.dictionary.doc2bow(self.df.tokens.values[i_article])]
...
def run_lda(texts, min_df=5, 	num_topics=20):
    ...
    # remove extremes (similar to the min/max df step used when creating the
    # tf-idf matrix)
...
def load_model_print_stats():
    ...
    lda_kw_dict = {
        'data': df, 'lda_model': lda_model, 'dictionary': dictionary}
...
def test_run():
    ...
    lda_model, topics_matrix, dictionary = run_lda(
        docs, min_df=5, num_topics=num_topic)
