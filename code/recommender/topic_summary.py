...
class TopicSummarizer(object):
    ...
    def add_features(self):
        ...
        self.df['pub_year'] = self.df.pub_date.map(
            lambda x: datetime.date.isocalendar(x)[0])
    ...
    def articles_num_by_month_dict(self):
        # articles per month
        dg = self.df[['pub_month_date_str', 'topic_sorted']].groupby(
            'pub_month_date_str')
    ...
    def articles_topic(self, outfile):  # pct articles per topic
        ...
        i_topic_display = 1
        # ,val in enumerate(x):
        for i_topic_display0, idx in enumerate(idx_sorted):
            if idx not in self.exclude_i_orig:
                f.write(str(i_topic_display) + ',' +
                        str(y[idx]) + ',' + str(idx) + ',' + self.topic_names[idx] + '\n')
                i_topic_display = i_topic_display + 1
        f.close()

    # example articles per topic per week
    def articles_bytopic_permonth(self, outfile):
        '''
...
if __name__ == '__main__':
    ...
    recommender.df_articles['dt'] = recommender.df_articles[
        'url'].map(lambda x: article_dt.get(x, None))
