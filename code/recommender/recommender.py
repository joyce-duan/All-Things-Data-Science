...
allds_home = config.get(
    'allDS_home', '/Users/joyceduan/Documents/git/All-Things-Data-Science/')
...
class Recommender(object):
    ...
    def calculate_recommendations(self, W, test_X2, input_name):
        ...
        cosine_similarities = linear_kernel(
            self.X_articles, test_X2[0:1]).flatten()

        cosin_simi_latent_topics = linear_kernel(
            self.W_articles, W[i_article, :]).flatten()
    ...
    def get_recommendation_dataframe(self, cosine_similarities, cosin_simi_latent_topics,
        ...
        for i in desc_sim_indexes[:self.max_rank]:
            if cosin_simi_latent_topics_rank[i] < self.max_rank and i_print < self.top_k_recommend:
                ...
                r['score'] = {'cosine_sim_tfidf': cosine_similarities[i], 'cosine_sim_tfidf_rank': cosine_similarities_rank[
                    i], 'cosine_simi_latent_topics': cosin_simi_latent_topics[i], 'cosine_sim_latent_topics_rank': cosin_simi_latent_topics_rank[i]}
                r['title'] = self.df_articles.iloc[i].title
                body_cleaned = ascii_text(
                    self.df_articles.iloc[i].body_text[:self.max_len_body_text])
                r['body_text'] = body_cleaned
                r['url'] = url
                r['topics'] = self.sorted_topics_articles[i][
                    0:self.top_k_topics]  # (itopic, topic_name, weight)
                r['relevance'] = min(
                    100, int(cosine_similarities[i] / self.max_cosine_sim_tfidf * 100))
    ...
    def process_input(self, cleaned_slack, dataset_name=''):
        ...
        W, tokenized_slacks2, test_X2 = self.topic_model.transform_bodytext2topics(
            [cleaned_slack], 1)
        print 'topics for input text'
        # print topic_model.sorted_topics_for_articles(W)
        # t1 = time.time() # time it
        # print "finished in  %4.4f min %s " %((t1-t0)/60,'topics of slack
        # message\n')
        ...
        for i in desc_feature_indexes[:top_n]:
            ...
            top_features_list.append(
                (features[i], test_X2_dense[i_article, i]))
...
def run_article_recom(df2):
    ...
    good_urls = ['http://www.bloomberg.com/news/articles/2015-06-04/help-wanted-black-belts-in-data',
                 'http://bits.blogs.nytimes.com/2015/04/28/less-noise-but-more-money-in-data-science/', 'http://bits.blogs.nytimes.com/2013/06/19/sizing-up-big-data-broadening-beyond-the-internet/'
                 ]
...
def pre_clean_text(func_tokenizer, data2):
    ...
    pre-clean text
        ...
        OUTPUT:
            - text_cleaned  str
    '''
    words_exclude = [u'text/html', 'pm', 'http' 'io', 'com', 'github', 'www',
                     'config', 'html', 'js', 'https', 'join', 'repo', 'chris', u'http', u'__init__']
    cleaned_slack = []
    # ipdb.set_trace()
...
def make_df_predictions(rating_predict, relevant_items_all, my_rec_engine, df2):
    ...
    for i_article in np.argsort(-1.0 * rating_predict)[:3]:
        ...
        # , relevant_items]
        sim_vec = my_rec_engine.item_sim_mat[i_article, :].flatten()
...
def process_input(cleaned_slack):
    ...
    '''
    # print type(cleaned_slack)
    W, tokenized_slacks2, test_X2 = recommender.topic_model.transform_bodytext2topics(
        [cleaned_slack], 1)
    print 'topics for input text'
    # print topic_model.sorted_topics_for_articles(W)
    # t1 = time.time() # time it
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        # '06-10.txt'  folder: /Users/joyceduan/documents/git/All-Things-Data-Science/data
