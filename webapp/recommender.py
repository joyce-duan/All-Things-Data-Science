...
if __name__ == '__main__':
	...
	print " %4.4fmin for %s " %((t1-t0)/60,'read data')
	print df.shape
	...
	t1 = time.time() # time it
	print " %4.4fmin for %s " %((t1-t0)/60,'tfidf')
	...
	t1 = time.time() # time it
	print " %4.4fmin for %s " %((t1-t0)/60,'fit and predict')

	t0 = t1
	dump_model(clf, vectorizer)
	t1 = time.time() # time it
	print " %4.4fmin for %s " %((t1-t0)/60,'dump model')

	t0 = t1
	vectorizer2, clf2 = read_model()
	t1 = time.time() # time it
	print " %4.4fmin for %s " %((t1-t0)/60,'read pickle')
	...
	t1 = time.time() # time it
	print " %4.4fmin for %s " %((t1-t0)/60,'predict ')
